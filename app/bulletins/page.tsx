...
export default function Bulletins() {
    ...
    const handleDelete = async (bulletin: bulletin) => {
      // Handle delete logic
      const res = await axios.post('/api/delete',{type:'bulletins',id:bulletin.id})
      if (res.status==200) {showToast.success({message:'Bulletin Deleted Succefully',duration:5000})
         const filteredBulletins = bulletins.filter(bull => bull.id != bulletin.id);
  ...
  useEffect(() => {
    const getbulletins = async () => {
        const res = await axios.post('http://localhost:3000/api/stores',{type:'bulletins'})
  ...
  , []);
   ...
   //else {
    ...
    }, [setBulletinname,bulletinname,setsearch,search]);
  return (
    <>
    <Navbar/>
      <DataTable<bulletin>
