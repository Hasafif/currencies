...
const BulletinForm = () => {
  ...
  const states = [
    'Damascus', 'Aleppo', 'Lattakia', 'Tartus', 'Homs',
    'Hama', 'Idlib', 'Daraa', 'Alsuida', 'Alkonitira',
    'Deri-Alzor', 'Al-raqaa', 'Al-hasaka'
  ];
  ...
  const getAvailableProductOptions = (currentProductId:number) => {
    ...
    return availableProducts.filter(product => !selectedProducts.includes(product));
  };
  ...
  const addProduct = () => {
    ...
    const remainingProducts = availableProducts.filter(p => !usedProducts.includes(p));
    
    if (remainingProducts.length > 0) {
      const newId = Math.max(...products.map(p => p.id)) + 1;
      setProducts(prev => [...prev, { id: newId, name: '', sale_price: '' , purchase_price:''}]);
    }
  };
  ...
  const validateForm = () => {
    ...
    console.log(prices);
    console.log(price)
    //console.log(prices[0].city)
    //console.log(prices[0].state)
   // console.log(prices[1].city)
    //console.log(prices[1].state)
    //console.log(prices[2].city)
   // console.log(prices[2].state)
   // console.log(storeInfo.state)
   // console.log(storeInfo.city)
    // Validate products
    ...
    return (!newErrors.storeInfo.storeName && !newErrors.storeInfo.state && !newErrors.storeInfo.city && !newErrors.products);
  };
  ...
  const handleSubmit = async (e:any) => {
    ...
    if (validateForm()) {
      ...
      const resp = await axios.post('http://localhost:3000/api/add_bulletin',
        {storeName:bulletinData.storeName,state:bulletinData.state,city:bulletinData.city, currencies:bulletinData.products})
     if (resp.status==200) {
       
      showToast.success({message:'Bulletin Added Sucessfully',duration:5000})
      }
      else {
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '20px' }}>
      <h2 style={{ textAlign: 'center', marginBottom: '20px' , color:"black"}}>Bulletin Information</h2>
          ...
          <div style={{ marginBottom: '15px', color:"black" }}>
            ...
            <input
              ...
              style={{
                width: '100%',
                padding: '8px',
                border: errors.storeInfo.storeName ? '1px solid red' : '1px solid #ccc',
                borderRadius: '4px'
              }}
            />
          ...
          <div style={{ marginBottom: '15px' , color:"black"}}>
            ...
            <select
              ...
              style={{
                width: '100%',
                padding: '8px',
                border: errors.storeInfo.state ? '1px solid red' : '1px solid #ccc',
                borderRadius: '4px'
              }}
            >
          ...
          <div style={{ marginBottom: '15px', color:"black" }}>
            ...
            <input
              ...
              style={{
                width: '100%',
                padding: '8px',
                border: errors.storeInfo.city ? '1px solid red' : '1px solid #ccc',
                borderRadius: '4px'
              }}
            />
            ...
            >
              <div style={{ marginBottom: '15px', color:"black" }}>
                ...
                <select
                  ...
                  style={{
                    width: '100%',
                    padding: '8px',
                    border: errors.products ? '1px solid red' : '1px solid #ccc',
                    borderRadius: '4px'
                  }}
                >
              ...
              <div style={{ marginBottom: '15px', color:"black" }}>
                ...
                <input
                  ...
                  onChange={(e) => handleProductChange(product.id, 'purchase_price', e.target.value)}
                  placeholder="0.00"
                  style={{
                    width: '100%',
                    padding: '8px',
